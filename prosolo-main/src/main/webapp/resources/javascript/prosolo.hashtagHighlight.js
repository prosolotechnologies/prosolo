 ...
 * 
 * 
 * Modified to add 'data' attribute with value of the matched element.
 */
...
(function($) {
	...
	$.fn.highlightHashtag = function(regex, options) {
		...
		options.onClickCallback = options.onClickCallback || $.noop;

		if (typeof regex === 'undefined' || regex.source === '') {

			$(this).find(options.tagType + '.' + options.className).each (
				function() {
					...
					if (searchnode.nodeType == 3) {
						...
						while (searchnode.data	&& (pos = searchnode.data.search(regex)) >= 0) {
							...
							if (match.length > 0) {
								...
								var hashtagVal = match.replace('#','');
								$(spannode).attr('data', hashtagVal);
								...
								options.onClickCallback(spannode, hashtagVal);
						...
						$(searchnode).highlightHashtag(regex, options);
					}
				});
			});
		}
