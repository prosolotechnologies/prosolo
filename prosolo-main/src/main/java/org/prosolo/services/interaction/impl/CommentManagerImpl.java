        ...
        try {
            ...
            StringBuilder query = new StringBuilder(
                    "SELECT DISTINCT comment.id, comment.description,  " +
                            ...
                            "user.id, user.name, user.lastname, user.avatarUrl, user.position, count(child.id) AS numberOfReplies, comment.resourceType " +
            ...
            for (Object[] row : res) {
                if (row != null) {
                    CommentData commentData = commentFactory.getCommentData(
                            ...
                            hasUserLikedComment(userId, (Long) row[0]), // likedByCurrentUser
                            (Long) row[4],            // commentedResourceId,
                            (CommentedResourceType) row[12],            // commentedResourceType,
...
//	}
    ...
    @Override
    @Transactional (readOnly = true)
    public Optional<CommentData> getCommentData(long commentId, long userId) {
        try {
            String query =
                    "SELECT comment " +
                    "FROM Comment1 comment " +
                    "LEFT JOIN comment.parentComment parent  " +
                    "WHERE comment.id = :commentId";

            Comment1 comment = (Comment1) persistence.currentManager()
                    .createQuery(query)
                    .setLong("commentId", commentId)
                    .uniqueResult();

            if (comment != null) {
                Comment1 parent = comment.getParentComment();
                CommentData parentCommentData = null;

                if (parent != null) {
                    parentCommentData = getCommentData(parent, userId, null, 0);
                }

                return Optional.of(getCommentData(
                        comment,
                        userId,
                        parentCommentData,
                        0));
            ...
            logger.error("Error", e);
            throw new DbConnectionException("Error loading comment data");
        }
    }
