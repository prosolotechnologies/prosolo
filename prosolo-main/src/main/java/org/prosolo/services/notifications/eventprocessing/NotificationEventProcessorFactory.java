...
public class NotificationEventProcessorFactory {
	...
	public NotificationEventProcessor getNotificationEventProcessor(Event event, Session session) {
		switch(event.getAction()) {
			/*
			 * A new comment was posted. If comment is posted on competence/activity page
			 * notify user that created competence/activity and all users that commented on
			 * that competence/activity.
			 */
			case Comment:
			case Comment_Reply:
				return new CommentEventProcessing(event, session, 
						notificationManager, notificationsSettingsManager, idEncoder, activityManager,
						commentManager);
			/*
			...
			case Dislike:
				return new LikeEventProcessing(event, session, 
						notificationManager, notificationsSettingsManager, idEncoder);
			default:
				return null;
		}
	}
