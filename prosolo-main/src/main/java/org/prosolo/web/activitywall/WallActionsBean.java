			...
			public void run() {
				...
				try {
					SocialActivityConfig config = activityWallActionsManager
							.hideNotification(socialActivity.getId(), loggedUser.getUser().getId(), 
									lcd, session);
					logger.debug("User "+loggedUser.getUser()+" hid social activity  "+socialActivity.getId()+" from the Activity Wall. Config id: "+config.getId());
					session.flush();
...
//		}
		...
		taskExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					activityWallActionsManager.deleteSocialActivity(loggedUser.getUser().getId(), 
							socialActivity, new LearningContextData(page, lContext, service), session);
					session.flush();
					logger.debug("User "+loggedUser.getUser().getId()+" deleted social activity " + socialActivity.getId() + " from the Activity Wall");
				} catch (DbConnectionException e) {
					logger.error("There was an error when user "+loggedUser.getUser()+" tried to delete social activity " + socialActivity.getId() + ". "+e);
				} finally {
					HibernateUtil.close(session);
				}
			}
	...
	public void enableComments(SocialActivityData1 socialActivity){
		...
		taskExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					activityWallActionsManager.enableComments(loggedUser.getUser().getId(), 
	...
	public void disableComments(final SocialActivityData1 socialActivity) {
		...
		taskExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					activityWallActionsManager.disableComments(loggedUser.getUser().getId(), 
							socialActivityId, new LearningContextData(page, lContext, service), session);
					session.flush();
