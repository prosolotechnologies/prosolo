...
public class PortfolioBean implements Serializable {
	...
	@Autowired
	private CompWallActivityConverter compWallActivityConverter;
	@Autowired
	private UploadManager uploadManager;
	@Autowired
	private LearnBean learningGoalsBean;
	@Autowired
	...
	public void setGoalDataToBeSentToGoals(final GoalData goalDataToBeSentToGoals, final String context) {
		...
		if (goalDataToBeSentToGoals != null) {
			taskExecutor.execute(new Runnable() {
				@Override
				public void run() {
					loggingNavigationBean.logServiceUse(ComponentName.SEND_TO_GOALS_DIALOG, "context", context,
	...
	public void setExCreditDataToDelete(ExternalCreditData exCreditDataToDelete) {
		...
		loggingNavigationBean.logServiceUse(ComponentName.DELETE_EXTERNAL_CREDIT_DIALOG, "context",
				"profile.credits." + exCreditDataToDelete.getId(), "exCreditId",
				String.valueOf(exCreditDataToDelete.getId()));
	}
