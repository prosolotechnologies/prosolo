...
public class BusinessCase_Test_2_14 extends BaseBusinessCase5 {
    ...
    private void askForAssessmentOfCompetency3(EventQueue events) throws Exception {
        CompetenceAssessmentData competenceAssessment1 = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6comp3Id, userRichardAnderson, userJosephGarcia.getId(), tokensSpentPerRequest);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).declineCompetenceAssessmentRequestAndGetEvents(competenceAssessment1.getAssessmentId(), createUserContext(userJosephGarcia)));
        CompetenceAssessmentData competenceAssessment2 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6comp3Id, userRichardAnderson, tokensSpentPerRequest, true);

        CompetenceAssessmentData competenceAssessment3 = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6comp3Id, userJosephGarcia, userRichardAnderson.getId(), tokensSpentPerRequest);
        ...
        try {
            ...
            CompetenceAssessment ca3 = (CompetenceAssessment) session.load(CompetenceAssessment.class, competenceAssessment3.getAssessmentId());
    ...
    private void askForAssessmentOfCompetency2(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6comp2Id, userJosephGarcia, userRichardAnderson.getId(), 0);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).declineCompetenceAssessmentRequestAndGetEvents(competenceAssessment.getAssessmentId(), createUserContext(userRichardAnderson)));

        Session session = (Session) ServiceLocator.getInstance().getService(DefaultManager.class).getPersistence().openSession();
        Transaction transaction = null;
        try {
            transaction = session.beginTransaction();
            CompetenceAssessment ca = (CompetenceAssessment) session.load(CompetenceAssessment.class, competenceAssessment.getAssessmentId());
    ...
    private void enrollStevenTurnerToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userStevenTurner, userRichardAnderson.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollJosephGarciaToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment1 = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userJosephGarcia, userRichardAnderson.getId(), tokensSpentPerRequest);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).declineCompetenceAssessmentRequestAndGetEvents(competenceAssessment1.getAssessmentId(), createUserContext(userRichardAnderson)));
    }
    ...
    private void enrollTimothyRiveraToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userTimothyRivera, userRichardAnderson.getId(), tokensSpentPerRequest);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).expireCompetenceAssessmentRequestAndGetEvents(competenceAssessment.getAssessmentId(), createUserContext(userNickPowell)));
    }
    ...
    private void enrollAnthonyMooreToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userAnthonyMoore, userRichardAnderson.getId(), tokensSpentPerRequest);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).acceptCompetenceAssessmentRequestAndGetEvents(competenceAssessment.getAssessmentId(), createUserContext(userRichardAnderson)));
        this.gradeCompetenceAssessmentWithRubric(events, competenceAssessment.getAssessmentId(), AssessmentType.PEER_ASSESSMENT, userRichardAnderson, rubricData.getLevels().get(2).getId());
        approveCompetenceAssessment(events, competenceAssessment.getAssessmentId(), competenceAssessment.getCredentialId(), competenceAssessment.getCompetenceId(), userRichardAnderson);
    }
...
//    }
    ...
    private void enrollSamanthaDellToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userSamanthaDell, userRichardAnderson.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollSheriLaureanoToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userSheriLaureano, userRichardAnderson.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollHelenCampbellToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, userRichardAnderson.getId(), tokensSpentPerRequest);
    }
