...
public class BusinessCase_Test_2_13 extends BaseBusinessCase5 {
    ...
    private void enrollRichardAndersonToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment1 = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(0).getCompetenceId(), userRichardAnderson, userKevinHall.getId(), tokensSpentPerRequest);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).declineCompetenceAssessmentRequestAndGetEvents(competenceAssessment1.getAssessmentId(), createUserContext(userKevinHall)));

        CompetenceAssessmentData competenceAssessment2 = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(1).getCompetenceId(), userRichardAnderson, userTimothyRivera.getId(), tokensSpentPerRequest);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).declineCompetenceAssessmentRequestAndGetEvents(competenceAssessment2.getAssessmentId(), createUserContext(userTimothyRivera)));
        ...
        try {
            ...
            CompetenceAssessment ca1 = (CompetenceAssessment) session.load(CompetenceAssessment.class, competenceAssessment1.getAssessmentId());
            ca1.setDateCreated(DateUtil.getNDaysBeforeNow(32));
            ca1.setQuitDate(DateUtil.getNDaysBeforeNow(31));

            CompetenceAssessment ca2 = (CompetenceAssessment) session.load(CompetenceAssessment.class, competenceAssessment2.getAssessmentId());
    ...
    private void enrollTaniaCorteseToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(1).getCompetenceId(), userTaniaCortese, userStevenTurner.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollSonyaElstonToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(1).getCompetenceId(), userSonyaElston, userTimothyRivera.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollAngelicaFallonToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(1).getCompetenceId(), userAngelicaFallon, userStevenTurner.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollIdaFritzToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment = askPeerForCompetenceAssessment(events, credential6Delivery1.getId(), credential6Competencies.get(1).getCompetenceId(), userIdaFritz, userTimothyRivera.getId(), tokensSpentPerRequest);
    }
    ...
    private void enrollHelenCampbellToDelivery6(EventQueue events) throws Exception {
        ...
        CompetenceAssessmentData competenceAssessment1 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).acceptCompetenceAssessmentRequestAndGetEvents(competenceAssessment1.getAssessmentId(), createUserContext(userRichardAnderson)));
        this.gradeCompetenceAssessmentWithRubric(events, competenceAssessment1.getAssessmentId(), AssessmentType.PEER_ASSESSMENT, userRichardAnderson, rubricData.getLevels().get(2).getId());
        approveCompetenceAssessment(events, competenceAssessment1.getAssessmentId(), userRichardAnderson);
        ...
        CompetenceAssessmentData competenceAssessment4 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);

        CompetenceAssessmentData competenceAssessment5 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).expireCompetenceAssessmentRequestAndGetEvents(competenceAssessment5.getAssessmentId(), createUserContext(userKevinHall)));

        CompetenceAssessmentData competenceAssessment6 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).acceptCompetenceAssessmentRequestAndGetEvents(competenceAssessment6.getAssessmentId(), createUserContext(userKennethCarter)));

        CompetenceAssessmentData competenceAssessment7 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).acceptCompetenceAssessmentRequestAndGetEvents(competenceAssessment7.getAssessmentId(), createUserContext(userAnthonyMoore)));
        this.gradeCompetenceAssessmentWithRubric(events, competenceAssessment7.getAssessmentId(), AssessmentType.PEER_ASSESSMENT, userAnthonyMoore, rubricData.getLevels().get(0).getId());
        extractResultAndAddEvents(events, ServiceLocator.getInstance().getService(AssessmentManager.class).declinePendingCompetenceAssessmentAndGetEvents(competenceAssessment7.getAssessmentId(), createUserContext(userAnthonyMoore)));

        CompetenceAssessmentData competenceAssessment8 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        CompetenceAssessmentData competenceAssessment9 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        CompetenceAssessmentData competenceAssessment10 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
        CompetenceAssessmentData competenceAssessment11 = askPeerFromPoolForCompetenceAssessment(events, credential6Delivery1.getId(), credential6CompetenciesHelenCampbell.get(0).getCompetenceId(), userHelenCampbell, tokensSpentPerRequest, true);
    }
