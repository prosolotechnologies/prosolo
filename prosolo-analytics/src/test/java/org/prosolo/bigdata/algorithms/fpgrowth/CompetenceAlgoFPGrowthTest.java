	...
	public void testRunAlgorithm() {
		...
		/*	for(List<Itemset> level:levels){
				...
				for(Itemset item:level){
					System.out.println("LEVEL 2:"+Arrays.toString(item.getItems())+" support:"+item.getAbsoluteSupport());
				}
			}*/
			...
			for(Itemset item:lastLevel){
				if(mostfrequent==null){
					mostfrequent=item;
				}else if(mostfrequent.getAbsoluteSupport()<item.getAbsoluteSupport()){
					mostfrequent=item;
				}
				
				System.out.println("LAST LEVEL:"+Arrays.toString(item.getItems())+" support:"+item.getAbsoluteSupport());
			}
			System.out.println("MOST FREQUENT:"+Arrays.toString(mostfrequent.getItems())+" support:"+mostfrequent.getAbsoluteSupport());
			...
			dbManager.insertFrequentCompetenceActivities(competenceid,activities);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		// STEP 2: Generating all rules from the set of frequent itemsets (based on Agrawal & Srikant, 94)
				...
				try {
					AssocRules assocRules=algoAgrawal.runAlgorithm(patterns,   databaseSize, minconf);
