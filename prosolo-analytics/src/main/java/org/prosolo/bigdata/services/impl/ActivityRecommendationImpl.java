...
public class ActivityRecommendationImpl implements ActivityRecommendation{
	...
	@Override
	public List<ActivityAccessCount> getRecommendedActivitiesForCompetence(Long competenceId, List<Long> alreadyAddedActivities, int limit){
		List<ActivityAccessCount> frequentActivities=null;
		if(alreadyAddedActivities.isEmpty()){
			frequentActivities=dbManager.findFrequentCompetenceActivities(competenceId);
		}else{
			frequentActivities=esSearch.findMatchingActivitiesForCompetenceInAssociationRules(alreadyAddedActivities, competenceId, limit);
		}
		return frequentActivities;
	}
	@Override
	public List<ActivityAccessCount> getRelatedActivitiesForActivity(
			Long competenceId, Long activityId, List<Long> alreadyAddedActivities, int limit) {
		List<ActivityAccessCount> relatedActivities=esSearch.findRelatedActivitiesForCompetenceAndActivityInAssociationRules(alreadyAddedActivities,competenceId, activityId, limit);
