...
public class LearningGoalsMostActiveUsersAnalyzer  implements Serializable{
	...
	private static Logger logger = Logger.getLogger(LearningGoalsMostActiveUsersAnalyzer.class);
	...
	public void analyzeLearningGoalsMostActiveUsersForDay(final long daysSinceEpoch){
		...
		List<UserLearningGoalActivitiesCount> activitiesCounters=dbManager.findUserLearningGoalActivitiesByDate(daysSinceEpoch);
		JavaRDD<UserLearningGoalActivitiesCount> activitiesCountersRDD = javaSparkContext.parallelize(activitiesCounters);
		JavaPairRDD<Long, Iterable<UserLearningGoalActivitiesCount>> actCountersByLearningGoals=activitiesCountersRDD.mapToPair(
			...
			public Tuple2<Long, UserLearningGoalActivitiesCount> call(
					UserLearningGoalActivitiesCount counter) throws Exception {
		...
		//sorting list of active users and selecting 10 most active for each learning goal
		JavaPairRDD<Long, Iterable<UserLearningGoalActivitiesCount>> actCountersByLearningGoalsSorted=actCountersByLearningGoals.mapValues(
				new Function<Iterable<UserLearningGoalActivitiesCount>,Iterable<UserLearningGoalActivitiesCount>>(){
					@Override
		...
		//iterate through rdd and store most active users for a day to cassandra (table mostactiveusersforlearninggoalbydate)
		actCountersByLearningGoalsSorted.foreach(new VoidFunction<Tuple2<Long,Iterable<UserLearningGoalActivitiesCount>>>(){
			/**
			 * 
			 */
				...
				Iterable<UserLearningGoalActivitiesCount> counters=tuple._2;
				 ...
				 data.add("date", new JsonPrimitive(daysSinceEpoch));
			   	...
			   	for(UserLearningGoalActivitiesCount counter:counters){
			   		mostactiveusers.add(String.valueOf(counter.getUserid()), new JsonPrimitive(counter.getCounter()));
			   	}
	...
	public void analyzeLearningGoalsMostActiveUsersForWeek(){
		...
		JavaSparkContext javaSparkContext = SparkLauncher.getSparkContext();
		JavaRDD<Long> daysToAnalyzeRDD=javaSparkContext.parallelize(daysToAnalyze);
		JavaRDD<List<MostActiveUsersForLearningGoal>> mostActiveUsersByDatePairRDD=daysToAnalyzeRDD.map(new Function<Long, List<MostActiveUsersForLearningGoal>>(){
			 
			@Override
			public List<MostActiveUsersForLearningGoal> call(Long date)
					throws Exception {
				List<MostActiveUsersForLearningGoal> mostActiveUsersForDate=dbManager.findMostActiveUsersForGoalsByDate(date);
  
				//Tuple2<Long, List<MostActiveUsersForLearningGoal>> tuple = new Tuple2<Long, List<MostActiveUsersForLearningGoal>>(date,mostActiveUsersForDate);
				return mostActiveUsersForDate;
			}});
		System.out.println("COUNT:"+mostActiveUsersByDatePairRDD.count());
		List<MostActiveUsersForLearningGoal> reducedList=mostActiveUsersByDatePairRDD.reduce(new Function2<List<MostActiveUsersForLearningGoal>,List<MostActiveUsersForLearningGoal>,List<MostActiveUsersForLearningGoal>>(){

			@Override
			public List<MostActiveUsersForLearningGoal> call(
					List<MostActiveUsersForLearningGoal> list1,
					List<MostActiveUsersForLearningGoal> list2) throws Exception {
				list1.addAll(list2);
 
				return list1;
			}});
 
		JavaRDD<MostActiveUsersForLearningGoal> mostActiveUsersRDD=javaSparkContext.parallelize(reducedList);
		JavaPairRDD<Long, Iterable<MostActiveUsersForLearningGoal>> mostActiveUsersByLearningGoalsPairRDD=mostActiveUsersRDD.mapToPair(
				...
				public Tuple2<Long, MostActiveUsersForLearningGoal> call(
						MostActiveUsersForLearningGoal arg0) throws Exception {
					// TODO Auto-generated method stub
					Tuple2<Long, MostActiveUsersForLearningGoal> tuple=new Tuple2<Long,MostActiveUsersForLearningGoal>(arg0.getLearninggoal(), arg0);
					return tuple;
				}
		}).groupByKey();
  		JavaPairRDD<Long, MostActiveUsersForLearningGoal> weeklyValuesByGoals= mostActiveUsersByLearningGoalsPairRDD.mapValues(
				new Function<Iterable<MostActiveUsersForLearningGoal>,MostActiveUsersForLearningGoal>(){
					/**
					 * 
					 */
					...
					public MostActiveUsersForLearningGoal call(
							...
							throws Exception {
						Iterator<MostActiveUsersForLearningGoal> iter=iterable.iterator();
						...
						while(iter.hasNext()){
							...
							MostActiveUsersForLearningGoal mostactive=iter.next();
							...
							for(Long user:users){
								...
								if(weeklyUserPoints.containsKey(user)){
									 userPointsCounter=usersForDay.get(user)+currPoints;
								}else{
							...
							System.out.println("USERS FOR WEEK:"+weeklyUserPoints.toString());
							List<Map.Entry<Long,Long>> topTen = new ArrayList<Map.Entry<Long,Long>>(weeklyUserPoints.entrySet());
							Collections.sort(topTen, new Comparator<Map.Entry<Long,Long>>() {
							  public int compare(
