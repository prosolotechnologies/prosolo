...
public class CompetenceActivitiesAssociationRulesAnalyzer implements Serializable{
	private static Logger logger = Logger.getLogger(CompetenceActivitiesAssociationRulesAnalyzer.class);
	...
	public void analyzeCompetenceActivitesAssociationRules(){
		...
		JavaRDD<Long> competencesRDD = javaSparkContext.parallelize(competences);
		JavaRDD<Long> competencesRDD2 =competencesRDD.map(new Function<Long,Long>(){

			@Override
			public Long call(Long competenceid) throws Exception {
				 CompetenceAlgoFPGrowth fpgrowth=new CompetenceAlgoFPGrowth(competenceid);
				 //String output = ".//fpgrowthoutput"+competenceid+".txt";
				 double minsupp = 0.6;
				 Itemsets patterns = null;
					try {
						int patternsCount=0;
						while(patternsCount<minPatternCount && minsupp>minMinsupp){
							logger.debug("PATTERNS:competence:"+competenceid+" patterns count:"+patternsCount+" minsupp:"+minsupp*2);
							patterns=fpgrowth.runAlgorithm(minsupp);
							patternsCount=patterns.getItemsetsCount();
							minsupp=minsupp/2;
						}
					 	
						logger.debug("FINISHED PATTERNS:competence:"+competenceid+" patterns count:"+patternsCount+" minsupp:"+minsupp*2);
					...
					// STEP 2: Generating all rules from the set of frequent itemsets (based on Agrawal & Srikant, 94)
							...
							try {
								...
								while(assocRulesCount<minAssocRulesCount && minconf>minMinconf){
									 assocRules=algoAgrawal.runAlgorithm(patterns, databaseSize, minconf);
									 assocRulesCount=assocRules.getRules().size();
									 logger.debug("ASSOCIATION RULES competence:"+competenceid+" rules number:"+assocRulesCount+" minconf:"+minconf);
									 minconf=minconf/2;
								}
								patterns=null;
								algoAgrawal.printStats();
								logger.debug("FINISHED ASSOCIATION RULES competence:"+competenceid+" rules number:"+assocRulesCount+" minconf:"+minconf*2);
								if(assocRules.getRules().size()>0){
									...
									for(AssocRule rule:assocRules.getRules()){
										System.out.println("RULE:"+Arrays.toString(rule.getItemset1())+" ==>"+Arrays.toString(rule.getItemset2())+" support:"+rule.getAbsoluteSupport()+" confidence:"+rule.getConfidence());
										 indexer.saveAssociationRulesForCompetence(competenceid, rule);
									}
								}
								logger.debug("ASSOCIATION RULES STORED IN ELASTICSEARCH for competence:"+competenceid+" rules number:"+assocRulesCount+" minconf:"+minconf*2);
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
