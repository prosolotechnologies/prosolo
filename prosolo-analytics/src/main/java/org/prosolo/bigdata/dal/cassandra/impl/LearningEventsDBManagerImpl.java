...
public class LearningEventsDBManagerImpl extends SimpleCassandraClientImpl implements LearningEventsDBManager {
	...
	private void prepareStatements() {
		...
		this.preparedStatements.put(GET_LEARNING_STATISTICS, this.getSession()
				.prepare(getLearningEventsData));
		
		String getLearningMilestonesData = "SELECT * FROM learningmilestones WHERE actorid=? and epochday >= ? AND epochday <= ?;";
		this.preparedStatements.put(GET_LEARNING_MILESTONES, this.getSession()
				.prepare(getLearningMilestonesData));
	}
	...
	private Optional<Statement> createEventNumberStatement(Map<Long, Integer> learningEventsPerDay, SessionRecord sessionRecord) {
		...
		//if more than one day-in-year/year is involved, wrap them in a counter batch
			...
			for(Map.Entry<Long, Integer> entry : learningEventsPerDay.entrySet()) {
				BoundStatement boundStatement = new BoundStatement(
						this.preparedStatements.get(UPDATE_LEARNING_EVENTS_COUNTER));
				statement.add(boundStatement.bind(Long.valueOf(entry.getValue()),sessionRecord.getUserId(),
						entry.getKey()));
			}
	...
	private Optional<Statement> createEventMilestonesStatement(Map<Long, List<String>> milestonesPerDay, SessionRecord sessionRecord) {
	...
	public List<LearningEventSummary> getLearningEventsData(long actorId, long epochDayFrom, long epochDayTo) {
		...
		List<LearningEventSummary> summaries = combineRows(eventCountersRows,milestoneRows);
		return summaries;
	}
