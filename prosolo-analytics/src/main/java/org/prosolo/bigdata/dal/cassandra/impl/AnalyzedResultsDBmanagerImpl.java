...
public class AnalyzedResultsDBmanagerImpl extends SimpleCassandraClientImpl
	...
	private void prepareStatements() {
		...
		this.queries.put("insert_frequentcompetenceactivities",
				insert_frequentcompetenceactivities);

		String find_frequentcompetenceactivities = "SELECT * FROM frequentcompetenceactivities WHERE competenceid=? ALLOW FILTERING;";
		this.queries.put("find_frequentcompetenceactivities",
				find_frequentcompetenceactivities);
	...
	@Override
	public List<ActivityAccessCount> findFrequentCompetenceActivities(
			long competenceId) {
		...
		if (row != null) {
			...
			for (Long id : ids) {
				ActivityAccessCount activityAccess = new ActivityAccessCount(
						id, competenceId, 0);
