...
public class StudentAssignEventDBManagerImpl extends SimpleCassandraClientImpl implements StudentAssignEventDBManager {
	...
	private static final String STUDENT_ASSIGN_EVENT_TABLE_NAME = "student_assign_events";
	...
	private void prepareStatements() {
		String updateStudentsAssignedAssigned = "UPDATE " + STUDENT_ASSIGN_EVENT_TABLE_NAME + " " +
				"USING TTL 97200 " +
		...
		String updateStudentsAssignedUnassigned = "UPDATE " + STUDENT_ASSIGN_EVENT_TABLE_NAME + " " +
				"USING TTL 97200 " +
		...
		String getCurrentTimestamp = "SELECT timestamp FROM " + TIMESTAMPS_TABLE_NAME + " " +
				"WHERE tablename=?;";
		this.preparedStatements.put(GET_CURRENT_TIMESTAMP, this.getSession()
				.prepare(getCurrentTimestamp));
		
		String updateCurrentTimestamp = "UPDATE " + TIMESTAMPS_TABLE_NAME + " " +
				"SET timestamp=? " +
				"WHERE tablename=?;";
	...
	private Long getCurrentTimestamp() {
		...
		try {
			...
			statement.setString(0, STUDENT_ASSIGN_EVENT_TABLE_NAME);
	...
	public boolean updateCurrentTimestamp(long bucket) {
		try {
			...
			statement.setString(1, STUDENT_ASSIGN_EVENT_TABLE_NAME);
