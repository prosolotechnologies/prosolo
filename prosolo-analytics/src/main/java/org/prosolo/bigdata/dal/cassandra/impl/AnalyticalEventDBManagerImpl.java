...
public class AnalyticalEventDBManagerImpl extends SimpleCassandraClientImpl implements Serializable, AnalyticalEventDBManager{
	...
	private void prepareStatements() {
		...
		this.queries.put("update_userlearninggoalactivity", update_userlearninggoalactivity);
		
		String find_userlearninggoalactivity = "SELECT * FROM userlearninggoalactivity WHERE date=? ALLOW FILTERING;";
		this.queries.put("find_userlearninggoalactivity", find_userlearninggoalactivity);
		
		String update_activityinteraction = "UPDATE activityinteraction  SET count=count+1 WHERE competenceid=? AND activityid=?;";
		this.queries.put("update_activityinteraction", update_activityinteraction);
		...
		this.queries.put("insert_targetcompetenceactivities", insert_targetcompetenceactivities);
		...
		this.queries.put("insert_mostactiveusersforlearninggoalbydate", insert_mostactiveusersforlearninggoalbydate);
		...
		this.queries.put("find_mostactiveusersforlearninggoalbydate", find_mostactiveusersforlearninggoalbydate);
		
		String find_activitiesforcompetence = "SELECT * FROM activityinteraction WHERE competenceid=? ALLOW FILTERING;";
		this.queries.put("find_activitiesforcompetence", find_activitiesforcompetence);
		
		String find_targetcompetenceactivities = "SELECT * FROM targetcompetenceactivities WHERE competenceid=? ALLOW FILTERING;";
		this.queries.put("find_targetcompetenceactivities", find_targetcompetenceactivities);
	...
	public void updateAnalyticsEventCounter(AnalyticsEvent event) {
		String statementName="update_"+event.getDataName().name().toLowerCase();
	...
	public void insertAnalyticsEventRecord(AnalyticsEvent event){
		String statementName="insert_"+event.getDataName().name().toLowerCase();
		BoundStatement boundStatement = new BoundStatement(
				this.preparedStatements.get(statementName));
		
		JsonObject data=event.getData();
		String str=this.queries.get(statementName);
		String paramsStr = str.substring(str.indexOf("(") + 1, str.indexOf(")"));
	...
	@Override
	public List<ActivityAccessCount> findAllActivitiesForCompetence(long competenceId, List<Long> ignoredActivities) {
		BoundStatement boundStatement = new BoundStatement(
				this.preparedStatements
						.get("find_activitiesforcompetence"));
		...
		for (Row row : rows) {
			if(!ignoredActivities.contains(row.getLong(0))){
				ActivityAccessCount actAccessCount=new ActivityAccessCount(row.getLong("activityid"),row.getLong("competenceid"), row.getLong("count"));
	...
	@Override
	public List<TargetCompetenceActivities> findAllActivitiesByTargetCompetenceForCompetence(long competenceId) {
		BoundStatement boundStatement = new BoundStatement(
				this.preparedStatements
						.get("find_targetcompetenceactivities"));
		boundStatement.setLong(0, competenceId);
		ResultSet rs = this.getSession().execute(boundStatement);
		List<Row> rows = rs.all();
		...
		for (Row row : rows) {
			...
			TargetCompetenceActivities tcActivities=new TargetCompetenceActivities(competenceid, targetcompetenceid, activities);
	...
	@Override
	public List<UserLearningGoalActivitiesCount> findUserLearningGoalActivitiesByDate(long date) {
		BoundStatement boundStatement = new BoundStatement(
				this.preparedStatements
						.get("find_userlearninggoalactivity"));
		...
		//	System.out.println("FOUND ROW:"+date+" "+learninggoalid+" "+userid+" "+count);
			UserLearningGoalActivitiesCount actCount=new UserLearningGoalActivitiesCount(learninggoalid, userid, dateSinceEpoch, count);
	...
	@Override
	public List<MostActiveUsersForLearningGoal> findMostActiveUsersForGoalsByDate(long date){
		BoundStatement boundStatement = new BoundStatement(
				this.preparedStatements
		...
		for(Row row: rows){
			...
			Type mapType = new TypeToken<Map<Long,Long>>() {}.getType();
			Map<Long,Long> mostactiveusersmap=new Gson().fromJson(row.getString("mostactiveusers"),mapType);
