...
public class CompetenceAlgoFPGrowth {
	private static Logger logger = Logger.getLogger(CompetenceAlgoFPGrowth.class);
	...
	public Itemsets runAlgorithm(double minsupp) throws IOException{
	  ...
	  //  }else{ // if the user want to save the result to a file
		...
		List<TargetCompetenceActivities> tCompActivities= dbManager.findAllActivitiesByTargetCompetenceForCompetence(competenceId);
	 ...
	 */
	private void fpgrowth(FPTree tree, long [] prefix, int prefixLength, int prefixSupport, Map<Long, Integer> mapSupport) throws IOException {
////		======= DEBUG ========
...
//		System.out.println(tree);
	 ...
	 */
	private void saveAllCombinationsOfPrefixPath(long[] itemsetBuffer, int position, 
			long[] prefix, int prefixLength, int support) throws IOException {
	 ...
	 */
	private void saveItemset(long [] itemset, int itemsetLength, int support) throws IOException {
		...
		/*if(writer != null){
		 ...
		 //	 patterns.addItemset(itemsetObj, itemsetLength);
		 	boolean isHighest=patterns.addHighestLevelItemset(itemsetObj, itemsetLength);
		 	if(isHighest){
		 		...
		 		esIndexer.saveFrequentCompetenceActivities(this.competenceId, activities);
		 	}
		//}
	}
	/**
	...
	public void printStats() {
		logger.debug("=============  FP-GROWTH STATS ============= competence:"+competenceId);
	...
	//	System.out.print(" Max memory usage: " + MemoryLogger.getInstance().getMaxMemory() + " mb \n");
		...
		logger.debug("Highest level of frequent itemsets:"+this.patterns.getCurrentLevel());
		logger.debug(" Frequent itemsets on highest level count : " + this.patterns.getHighestLevelItemsetsCount()); 
