...
public abstract class AbstractOrderedItemset extends AbstractItemset {
	 ...
	 * @return the item
	 */
	public abstract Long get(int position);
	...
	public double getRelativeSupport(int nbObject) {
		// Divide the absolute support by the number of transactions to get the
		// relative support
	...
	/**
	 * This methods checks if another itemset is contained in this one. The
	 * method assumed that items are lexically ordered in itemsets.
	 * 
	 * @param itemset2
	 *            the other itemset
	 ...
	 * itemset except the last item and that itemset2 is lexically smaller than
	 * this itemset. If all these conditions are satisfied, this method return
	 * the last item of itemset2. Otherwise it returns null.
	 * 
	 * @return the last item of itemset2, otherwise, null.
	 * */
	public Long allTheSameExceptLastItem(AbstractOrderedItemset itemset2) {
		// if these itemsets do not have the same size, return null
		if (itemset2.size() != this.size()) {
			return null;
		}
		// We will compare all items one by one starting from position i =0 to
		// size -1
		for (int i = 0; i < this.size(); i++) {
			// if this is the last position
			if (i == this.size() - 1) {
				// We check if the item from this itemset is be smaller (lexical
				// order)
