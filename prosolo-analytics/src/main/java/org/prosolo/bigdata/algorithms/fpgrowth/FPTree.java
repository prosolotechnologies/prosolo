...
public class FPTree {
	 ...
	 * @param newNode
	 *            the new node thas has been inserted.
	 */
	private void fixNodeLinks(Long item, FPNode newNode) {
		...
		if (lastNode != null) {
			// if not null, then we add the new node to the node link of the
			// last node
	 ...
	 */
	void addPrefixPath(List<FPNode> prefixPath,
			Map<Long, Integer> mapSupportBeta, int relativeMinsupp) {
	 ...
	 * 
	 * @param mapSupport
	 *            the frequencies of each item (key: item value: support)
	 */
