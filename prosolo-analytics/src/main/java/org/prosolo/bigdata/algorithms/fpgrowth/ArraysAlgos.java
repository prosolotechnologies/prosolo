...
public class ArraysAlgos {
	 ...
	 * @return the copy
	 */
	public static long[] cloneItemSetMinusOneItem(long[] itemset, long item) {
		// create the new itemset
		long[] newItemset = new long[itemset.length - 1];
		int i = 0;
	 ...
	 * @return the copy
	 */
	public static long[] cloneItemSetMinusAnItemset(long[] ls, long[] hm_P_1) {
		// create a new itemset
		long[] newItemset = new long[ls.length - hm_P_1.length];
		int i = 0;
		// for each item of this itemset
		for (int j = 0; j < ls.length; j++) {
			// copy the item except if it is not an item that should be excluded
			if (Arrays.binarySearch(hm_P_1, ls[j]) < 0) {
				newItemset[i++] = ls[j];
			}
		}
		return newItemset; // return the copy
	...
	/**
	 * This method checks if this itemset is the same as another itemset except
	 * for the last item. It assumes that both itemsets have the same length.
	 * 
	 * @param itemset1
	 *            the first itemset
	 * @param itemset2
	...
	/**
	 * This method performs the intersection of two sorted arrays of integers
	 * and return a new sorted array.
	 * 
	 * @param a
	 *            the first array
	 * @param b
	 ...
	 */
	public static long[] intersectTwoSortedArrays(long[] array1, long[] array2) {
		// create a new array having the smallest size between the two arrays
		final int newArraySize = (array1.length < array2.length) ? array1.length
				: array2.length;
	...
	/**
	 * Check if an itemset contains another itemset. It assumes that itemsets
	 * are sorted according to the lexical order.
	 * 
	 * @param itemset1
	 *            the first itemset
	 * @param itemset2
	 ...
	 */
	public static boolean containsOrEquals(Integer itemset1[],
			Integer itemset2[]) {
		...
		loop1: for (int i = 0; i < itemset2.length; i++) {
			...
			for (int j = 0; j < itemset1.length; j++) {
				// if the current item in itemset1 is equal to the one in
				// itemset2
	...
	/**
	 * This method checks if an item "item" is in the itemset "itemset". It
	 * assumes that items in the itemset are sorted in lexical order and that
	 * the largest item in the itemset is known.
	 * 
	 * @param itemset
	 *            an itemset
	 * @param item
	 ...
	 *            the largest item in the itemset
	 * @return returnt true if the item appears in the itemset
	 */
	public static boolean containsLEX(Integer itemset[], Integer item,
			int maxItemInArray) {
		...
		for (Integer itemI : itemset) {
			...
			// the method returns false because of the lexical order in the
			// itemset.
			else if (itemI > item) {
				return false; // <-- xxxx
			}
	...
	/**
	 * Method to compare two sorted list of integers and see if they are the
	 * same, while ignoring an item from the second list of integer. This
	 * methods is used by some Apriori algorithms.
	 * 
	 * @param itemset1
	 *            the first itemset
	 * @param itemsets2
	 *            the second itemset
	 * @param posRemoved
	 *            the position of an item that should be ignored from "itemset2"
	...
	public static boolean containsLEXPlus(int[] itemset, int item) {
		...
		for (int i = 0; i < itemset.length; i++) {
			...
			if (itemset[i] == item) {
				...
				// then return true because if if the item "item" was added it
	 ...
	 * lexical order.
	 */
	public static Comparator<long[]> comparatorItemsetSameSize = new Comparator<long[]>() {
		@Override
		/**
		 ...
		 */
		public int compare(long[] itemset1, long[] itemset2) {
			// for each item in the first itemset
			for (int i = 0; i < itemset1.length; i++) {
