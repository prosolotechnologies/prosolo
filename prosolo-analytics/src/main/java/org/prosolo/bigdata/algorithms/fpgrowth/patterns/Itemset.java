...
public class Itemset extends AbstractOrderedItemset {
	 ...
	 */
	public Itemset(long item) {
		itemset = new long[] { item };
	}
	 ...
	 * 
	 * @param items
	 *            a list of Integer representing items in the itemset
	 * @param support
	 *            the support of the itemset
	 */
	public Itemset(List<Long> itemset, int support) {
		this.itemset = new long[itemset.size()];
		int i = 0;
		for (Long item : itemset) {
			this.itemset[i++] = item.longValue();
		}
	...
	public Itemset cloneItemSetMinusAnItemset(Itemset itemsetToNotKeep) {
		...
		long[] newItemset = new long[itemset.length - itemsetToNotKeep.size()];
		int i = 0;
	...
	public Itemset intersection(Itemset itemset2) {
		long[] intersection = ArraysAlgos.intersectTwoSortedArrays(
