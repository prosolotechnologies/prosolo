...
import org.prosolo.bigdata.algorithms.fpgrowth.ArraysAlgos;
...
public class AlgoAgrawalFaster94 {
	private static Logger logger = Logger.getLogger(AlgoAgrawalFaster94.class);
	 ...
	 */
	public AssocRules runAlgorithm(Itemsets patterns, int databaseSize,
		...
		// start the algorithm
		return runAlgorithm(patterns, databaseSize);
	}
	...
	/**
	 * Run the algorithm for generating association rules from a set of
	 * itemsets.
	 * 
	 * @param patterns
	 *            the set of itemsets
	 * @param output
	 *            the output file path. If null the result is saved in memory
	 *            and returned by the method.
	 * @param databaseSize
	 *            the number of transactions in the original database
	 * @return the set of rules found if the user chose to save the result to
	 *         memory
	 * @throws IOException
	 *             exception if error while writting to file
	 */
	private AssocRules runAlgorithm(Itemsets patterns, int databaseSize)
			throws IOException {
		...
		// We do this for optimization purposes. If the itemsets are sorted, it
		// allows to
		// perform two optimizations:
		// 1) When we need to calculate the support of an itemset (in the method
		// "calculateSupport()") we can use a binary search instead of browsing
		// the whole list.
		// 2) When combining itemsets to generate candidate, we can use the
		// lexical order to avoid comparisons (in the method
		// "generateCandidates()").
		...
		for (List<Itemset> itemsetsSameSize : patterns.getLevels()) {
			...
			Collections.sort(itemsetsSameSize, new Comparator<Itemset>() {
				...
				public int compare(Itemset o1, Itemset o2) {
					...
					return ArraysAlgos.comparatorItemsetSameSize.compare(
							o1.getItems(), o2.getItems());
				}
			});
		}
		// END OF SORTING
		...
		for (int k = 2; k < patterns.getLevels().size(); k++) {
			for (Itemset lk : patterns.getLevels().get(k)) {
				...
				List<long[]> H1_for_recursion = new ArrayList<long[]>();

				// For each itemset "itemsetSize1" of size 1 that is member of
				// lk
				for (long item : lk.getItems()) {
					long itemsetHm_P_1[] = new long[] { item };

					// make a copy of lk without items from hm_P_1
					long[] itemset_Lk_minus_hm_P_1 = ArraysAlgos
							.cloneItemSetMinusOneItem(lk.getItems(), item);

					// Now we will calculate the support and confidence
					// of the rule: itemset_Lk_minus_hm_P_1 ==> hm_P_1
					int support = calculateSupport(itemset_Lk_minus_hm_P_1); // THIS
																				// COULD
																				// BE
					// OPTIMIZED ?
					...
					// itemset_Lk_minus_hm_P_1 ==> hm_P_1
					if (usingLift) {
						// if we want to calculate the lift, we need the support
						// of hm_P_1
						supportHm_P_1 = calculateSupport(itemsetHm_P_1); // if
																			// we
																			// want
																			// to
																			// calculate
																			...
																			// this.
						// calculate the lift
						double term1 = ((double) lk.getAbsoluteSupport())
								/ databaseSize;
					...
					// If we are here, it means that the rule respect the
					// minconf and minlift parameters.
					...
					// Then we keep the itemset hm_P_1 to find more rules using
					// this itemset and lk.
					H1_for_recursion.add(itemsetHm_P_1);
					// ================ END OF WHAT I HAVE ADDED
				}
				// Finally, we make a recursive call to continue explores rules
				// that can be made with "lk"
		...
		// Return the rules found if the user chose to save the result to memory
		// rather than a file.
	...
	/**
	 * The ApGenRules as described in p.14 of the paper by Agrawal. (see the
	 * Agrawal paper for more details).
	 * 
	 * @param k
	 *            the size of the first itemset used to generate rules
	 * @param m
	 *            the recursive depth of the call to this method (first time 1,
	 *            then 2...)
	 * @param lk
	 *            the itemset that is used to generate rules
	 * @param Hm
	 *            a set of itemsets that can be used with lk to generate rules
	 * @throws IOException
	 *             exception if error while writing output file
	 */
	private void apGenrules(int k, int m, Itemset lk, List<long[]> Hm)
			throws IOException {
		...
		if (k > m + 1) {
			...
			// recursive call
			List<long[]> Hm_plus_1_for_recursion = new ArrayList<long[]>();

			// generate candidates using Hm
			List<long[]> Hm_plus_1 = generateCandidateSizeK(Hm);

			// for each such candidates
			for (long[] hm_P_1 : Hm_plus_1) {

				// We subtract the candidate from the itemset "lk"
				long[] itemset_Lk_minus_hm_P_1 = ArraysAlgos
						.cloneItemSetMinusAnItemset(lk.getItems(), hm_P_1);
				...
				// minlift.
				if (usingLift) {
					// if we want to calculate the lift, we need the support of
					// Hm+1
					...
					lift = term1
							/ (term2 * ((double) supportHm_P_1 / databaseSize));
				...
				// requirements,
				// so we can output it
				saveRule(itemset_Lk_minus_hm_P_1, support, hm_P_1,
						supportHm_P_1, lk.getAbsoluteSupport(), conf, lift);
	 ...
	 * of the same size. Because patterns are sorted by lexical order, we use a
	 * binary search. This is MUCH MORE efficient than just browsing the full
	 * list of patterns.
	 * 
	 * @param itemset
	 *            the itemset.
	 ...
	 */
	private int calculateSupport(long[] itemset) {
		// We first get the list of patterns having the same size as "itemset"
		List<Itemset> patternsSameSize = patterns.getLevels().get(
				itemset.length);
		//
		// We perform a binary search to find the position of itemset in this
		// list
		int first = 0;
		...
		while (first <= last) {
			...
			long[] itemsetMiddle = patternsSameSize.get(middle).getItems();

			int comparison = ArraysAlgos.comparatorItemsetSameSize.compare(
					itemset, itemsetMiddle);
			if (comparison > 0) {
		...
		// The following line will not happen because in the context of this
		// algorithm, we will
		// always search for itemsets that are frequent and thus will be in the
		// list of patterns.
		// We just put the following line to avoid compilation error and detect
		// if the error if this
		// case was ever to happen.
		return 0;
		// throw new
		// RuntimeException("INVALID SUPPORT - THIS SHOULD NOT HAPPEN BECAUSE ALL ITEMSETS HAVE TO BE FREQUENT");
	}
	 ...
	 * also used by the Apriori algorithm for generating candidates. Note that
	 * this method is very optimized. It assumed that the list of itemsets
	 * received as parameter are lexically ordered.
	 * 
	 * @param hm
	 *            a set of itemsets of size k-1
	 * @return a set of candidates
	 */
	protected List<long[]> generateCandidateSizeK(List<long[]> hm) {
		// create a variable to store candidates
		List<long[]> candidates = new ArrayList<long[]>();

		// For each itemset I1 and I2 of level k-1
		loop1: for (int i = 0; i < hm.size(); i++) {
			long[] itemset1 = hm.get(i);
			loop2: for (int j = i + 1; j < hm.size(); j++) {
				long[] itemset2 = hm.get(j);
				...
				long lastItem1 = itemset1[itemset1.length - 1];
				long lastItem2 = itemset2[itemset2.length - 1];
				long newItemset[];
				if (lastItem1 < lastItem2) {
					// Create a new candidate by combining itemset1 and itemset2
					newItemset = new long[itemset1.length + 1];
	...
	public void printStats() {
		logger.debug("=============  ASSOCIATION RULE GENERATION v0.96f- STATS =============");
		logger.debug(" Number of association rules generated : " + ruleCount);
	 ...
	 * 
	 * @param itemset1
	 *            left itemset of the rule
	 * @param supportItemset1
	 *            the support of itemset1 if known
	 * @param itemset2
	 *            right itemset of the rule
	 * @param supportItemset2
	 *            the support of itemset2 if known
	 ...
	 */
	protected void saveRule(long[] itemset1, int supportItemset1,
			long[] itemset2, int supportItemset2, int absoluteSupport,
			double conf, double lift) throws IOException {
		ruleCount++;
		...
		if (writer != null) {
			...
			rules.addRule(new AssocRule(itemset1, itemset2, supportItemset1,
					absoluteSupport, conf, lift));
		}
	}
