...
public class AlgoAgrawalFaster94 {
	 ...
	 */
	public AssocRules runAlgorithm(Itemsets patterns,   int databaseSize, double minconf) throws IOException {
		// save the parameters
	 ...
	 * @throws IOException exception if error writing to the output file
	 */
	public AssocRules runAlgorithm(Itemsets patterns,  int databaseSize, double minconf,
		...
		for (int k = 2; k < patterns.getLevels().size(); k++) {
			for (Itemset lk : patterns.getLevels().get(k)) {
				...
				for(long item : lk.getItems()) {
					...
					long[] itemset_Lk_minus_hm_P_1 = ArraysAlgos.cloneItemSetMinusOneItem(lk.getItems(), item);
	...
	private void apGenrules(int k, int m, Itemset lk, List<long[]> Hm)
		...
		if (k > m + 1) {
			...
			for (long[] hm_P_1 : Hm_plus_1) {
				...
				long[] itemset_Lk_minus_hm_P_1 =  ArraysAlgos.cloneItemSetMinusAnItemset(lk.getItems(), hm_P_1);
...
//        throw new RuntimeException("INVALID SUPPORT - THIS SHOULD NOT HAPPEN BECAUSE ALL ITEMSETS HAVE TO BE FREQUENT");
	 ...
	 * 
	 * @param hm  a set of itemsets of size k-1
	 ...
	 */
	protected void saveRule(long[] itemset1, int supportItemset1, long[] itemset2, int supportItemset2,
