...
public class ElasticSearchConnector {
	...
	public static Client getClient() throws IndexingServiceNotAvailable {
		if (client == null) {
			if (CommonSettings.getInstance().config.elasticSearch.type
					.equals("local")) {
				client = getLocalClient();
			} else if (CommonSettings.getInstance().config.elasticSearch.type
					.equals("server")) {
				client = getESClient();
			} else if (CommonSettings.getInstance().config.elasticSearch.type
					.equals("cloud-aws")) {
	...
	private static Client getESClient() throws IndexingServiceNotAvailable {
		...
		try {
			...
			throw new IndexingServiceNotAvailable(
					"ElasticSearch node is not available at:"
							+ esHosts.toString() + ex);
		} catch (Exception ex) {
			logger.error("Exception for cluster:"
					+ elasticSearchConfig.clusterName, ex);
			return null;
		}
		return client;
	}
	...
	private static Client getLocalClient() {
		...
		Node node = nodeBuilder().local(true)
				.settings(elasticsearchSettings.build()).client(false)
				.data(true).node();
		client = node.client();
	...
	private static Client getAWSClient() {
		...
		try {
			...
			settings.put("cloud.aws.access_key",
					elasticSearchConfig.awsConfig.accessKey);
			settings.put("cloud.aws.secret_key",
					elasticSearchConfig.awsConfig.secretKey);
			settings.put("discovery.ec2.availability_zones",
					elasticSearchConfig.awsConfig.region);
			settings.put("discovery.type", "ec2");
			settings.put("discovery.ec2.groups",
					elasticSearchConfig.awsConfig.groups);
			...
			logger.error("NoNodeAvailableException for cluster:"
					+ elasticSearchConfig.clusterName, ex);
			return null;
